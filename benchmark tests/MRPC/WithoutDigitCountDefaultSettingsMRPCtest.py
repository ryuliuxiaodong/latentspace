...
if __name__ == "__main__":
    ...
    f2 = open(r'/tuned classifiers/default settings/without digit-count/MRPC(WithoutDigitCount).model', 'rb')
    s2 = f2.read()
    classifier = pickle.loads(s2)

    test_sentence_pair_list = []
    test_label_list = []
    test_file = open(r"/msr_paraphrase_test.txt", 'r', encoding='utf-8')
    ...
    for sent1, sent2 in test_sentence_pair_list:
        latent_s = defaultSettings.get_features_for_PI_task(sent1, sent2, with_digit_count=False)
