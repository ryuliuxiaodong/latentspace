...
def get_latent_semantics_paraphrase_space(fasttext, matrix, sentence, diff, word_len, count, j_dist):
...
def get_latent_semantics_nonparaphrase_space(fasttext, matrix, sentence, diff, word_len, count, j_dist):
    activation_function = lambda x: numpy.tanh(x)
    tokens = word_tokenize(sentence)
    if len(tokens) == 0:
        return 0
...
if __name__ == "__main__":
    ...
    while flag <= 800:
        ...
        matrix_file = open(r'D:\Model\pretrained\latentspaes\w\paraphrase_latent_space_with_digitcount_at_epochs_' + str(flag) + '.csv', 'r')
        matrix_table = matrix_file.readlines()
        matrix_file.close()
        matrix_file = open(r'D:\Model\pretrained\latentspaes\w\non-paraphrase_latent_space_with_digitcount_at_epochs_' + str(flag) + '.csv', 'r')
        ...
        for sentence1, sentence2 in paraphrase_sentence_pairs:
            ...
            latent_semantics1_p = get_latent_semantics_paraphrase_space(fasttext, matrix, sentence1, diff, word_len1, count, j_dist)
            latent_semantics2_p = get_latent_semantics_paraphrase_space(fasttext, matrix, sentence2, diff, word_len2, count, j_dist)
            latent_semantics1_np = get_latent_semantics_nonparaphrase_space(fasttext, matrix2, sentence1, diff, word_len1, count, j_dist)
            latent_semantics2_np = get_latent_semantics_nonparaphrase_space(fasttext, matrix2, sentence2, diff, word_len2, count, j_dist)
    ...
    plt.grid()
    plt.title('with digit-count scheme')
    plt.xlabel('epochs')
