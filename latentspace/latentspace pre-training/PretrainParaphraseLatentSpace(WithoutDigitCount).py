...
if __name__ == "__main__":
    ...
    ### initialize paraphrase latent space (projection matrix) with the length of input layer, output layer and learning rate
    bow = bowtf.BOWTF(1500, 100, 0.0005)

    paraphrase_file = open(r"/msr_paraphrase_train_1.csv", 'r', encoding='utf-16')
    ...
    for e in range(epochs):
        ...
        if (e+1) % 50 == 0:
            ...
            bow.save_matrix_to_csv(r'/paraphrase_latent_space_without_digitcount_at_epochs_' + str(e+1) + '.csv')

    ### visualize the error loss of all epoches
    ax = plt.subplot()
    positions = numpy.arange(1, epochs + 1)
    ax.bar(positions, errors, 0.5)
    plt.show()