...
if __name__ == "__main__":
    ...
    train_feature_list_adjusted_settings_with_digitcount = []
    for sent1, sent2 in train_sentence_pair_list:
        latent_s = adjustedSettings.get_features_for_PI_task(sent1, sent2)
        train_feature_list_adjusted_settings_with_digitcount.append(latent_s)


    ###initialize dataloaders
    train_inputs = torch.tensor(train_feature_list_adjusted_settings_with_digitcount).float()
    train_labels = torch.tensor(train_label_list)
    train_dataset = TensorDataset(train_inputs, train_labels)
    train_dataloader_adjusted_settings_with_digitcount = DataLoader(
        train_dataset,
        sampler=RandomSampler(train_dataset),
        batch_size=32
    )

    eval_inputs = torch.tensor(train_feature_list_adjusted_settings_with_digitcount).float()
    eval_labels = torch.tensor(train_label_list)
    eval_dataset = TensorDataset(eval_inputs, eval_labels)
    eval_dataloader_adjusted_settings_with_digitcount = DataLoader(
        eval_dataset,
    ...
    ###train train_dataloader_adjusted_settings_with_digitcount
    classifier = Classifier()
    classifier.cuda()

    epochs = 100
    optimizer = AdamW(classifier.parameters(), lr=5e-5, eps=1e-8)
    total_steps = len(train_dataloader_adjusted_settings_with_digitcount) * epochs
    ...
    for epoch_i in range(0, epochs):
        ...
        for step, batch in enumerate(train_dataloader_adjusted_settings_with_digitcount):
            ...
            if step % 40 == 0 and not step == 0:
                ...
                print('  Batch {:>5,}  of  {:>5,}.    Elapsed: {:}.'.format(step, len(train_dataloader_adjusted_settings_with_digitcount), elapsed))
                avg_train_loss = total_train_loss / step
                print("  40-step training loss: {0:.2f}".format(avg_train_loss))

        avg_train_loss = total_train_loss / len(train_dataloader_adjusted_settings_with_digitcount)
        ...
        for batch in eval_dataloader_adjusted_settings_with_digitcount:
